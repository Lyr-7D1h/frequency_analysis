...
crepeats = ['SS', 'EE', 'TT', 'FF', 'LL', 'MM', 'OO']
cgrams = ['E', 'T', 'A', 'O', 'N', 'R', 'I', 'S','H', 'D', 'L','F','C','M','U','G','P','W','B','V','K','J','X','Z','Q']
cbigrams = ['TH', 'HE', 'AN', 'RE', 'ER', 'IN', 'ON', 'AT', 'ND', 'ST', 'ES', 'EN', 'OF', 'TE', 'ED', 'OR','TI', 'HI', 'AS', 'TO']
ctrigrams = ['THE', 'AND', 'THA','ENT', 'ING', 'ION', 'TIO', 'FOR']

# slowly fill in char map
char_map = {
'S': 'e',
'J': 't',
'D': 'h',
'Q': 'a',
# try out more letters
}
...
# get reoccuring sequences of length in text
def get_seq_count_of_text(length, text):
    count = {}
...
# get sequences of partially decoded text
def get_sequences(partialctext):
    count = [] 
    sequel = ''
    for char in partialctext: 
        if char.islower():
            sequel += char
        else:
            if len(sequel) >= 2:
...
# if you have patterns that fit in eachother and have same count use biggest
def get_patterns(text):
    patterns = {} 
    current_pattern = ''
    for ichar in range(len(text)):
        current_pattern += text[ichar]
        # print(str(ichar / len(text) * 100) + '%')
        for pattern_size in range(len(text) - 1):
            check_pattern = ''
            min = ichar
            max = len(text) if ichar + pattern_size > len(text) else ichar + pattern_size
            if max - min > 20:
                continue
            for ipattern in range(min, max): 
                check_pattern += text[ipattern]
            # print(check_pattern)
            if len(check_pattern) > 3:
                has_lowercase = False
                for char in check_pattern:
                    if char.islower():
                        has_lowercase = True
                if has_lowercase and check_pattern in current_pattern:
                    # print('adding pattern: ' + check_pattern)
    ...
    # filter if same count only get biggest
    filtered_patterns = {}
    for iteration in grouped_patterns:
        ipatterns = grouped_patterns[iteration]
        filtered_ipatterns = []
        # for each pattern in iteration
        for cpati in range(len(ipatterns)): 
            fits = False
            for opati in range(len(ipatterns)):
                # skip current
                if cpati != opati:
                   if ipatterns[cpati] in ipatterns[opati]:
                       fits = True
            if not fits:
                filtered_ipatterns.append(ipatterns[cpati])

        for pat in filtered_ipatterns:
            filtered_patterns[pat] = iteration    

    return sorted(filtered_patterns.items(), key=lambda x: x[::-1], reverse=True)


def format_offset(pattern, pattern_index, common_patterns):
    try:
        offset = common_patterns.index(pattern) - pattern_index
        if abs(offset) > 2:
            return '\033[91m[Off ' + str(offset) +']\033[0m'
    except:
        return '\033[91m[NF]\033[0m'
    return ''

def pretty_format(sorted_list, optional_pattern_size=None):
    def format(tuple):
        (index, set) = tuple
        word = ''
        translated_chars = []
        for char in set[0]:
            if char in char_map:
                translated_chars.append(char_map[char])
        ...
        # If Complete check offset with most common
        if len(set[0]) == len(translated_chars):
            pattern = ''.join(translated_chars).upper()
            if optional_pattern_size == 0:
                word += format_offset(pattern, index, crepeats)
            elif optional_pattern_size == 1:
                word += format_offset(pattern, index, cgrams)
            elif optional_pattern_size == 2:
                word += format_offset(pattern, index, cbigrams)
            elif optional_pattern_size == 3:
                word += format_offset(pattern, index, ctrigrams)
            else:
                raise "Wrong optional argument should be between 0-3"
        return word + '[' + str(set[1]) + ']'


    return ' '.join(map(format, enumerate(sorted_list)))


grams = get_seq_count_of_text(1, ciphertext)
repeats = get_repeats(ciphertext)
bigrams = get_seq_count_of_text(2, ciphertext)[:20]
trigrams = get_seq_count_of_text(3, ciphertext)[:20]

# only show unused grams
print('Repeats:')
print(pretty_format(repeats, 0))
print('Grams:')
print(pretty_format(grams, 1))
print('Bigrams:')
print(pretty_format(bigrams, 2))
print('Trigrams:')
print(pretty_format(trigrams, 3))
print()
...
print('Words: ')
print(pretty_format(count_words( get_sequences(plaintext))))
patterns = get_patterns(plaintext)[0:50]
print()
...
ichar = 0
arg_count = 0
while ichar < len(plaintext):
    # check if char in pattern
    for pattern_length in range(2, 20):
        check_pattern = ''
        # max is length of plaintext or char position + length
        pmax = len(plaintext) if ichar + pattern_length > len(plaintext) else ichar + pattern_length
        for i in range(ichar, pmax):
            check_pattern += plaintext[i] 

        # check if search argument
        if len(sys.argv) > 1 and check_pattern == sys.argv[1]:
            arg_count += 1
            for char in check_pattern:
                colored_plaintext += '\033[91m' + char + '\033[0m'
            ichar += len(check_pattern)
            continue
        # if it is a pattern add colored characters and continue
        elif color_patterns and check_pattern in patterns:
            for char in check_pattern:
                colored_plaintext += '\033[92m' + char + '\033[0m'
            ichar += len(check_pattern)
            continue
    if plaintext[ichar].islower():
        colored_plaintext += '\033[93m' + plaintext[ichar] + '\033[0m'
    else:
        colored_plaintext += plaintext[ichar]
    ichar += 1
...
if len(sys.argv) > 1:
    print()
    print('Found: "' + sys.argv[1] + '" ' + str(arg_count) + ' times')
